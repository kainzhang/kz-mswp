...
$(function () {
    $('#mswp-container').mouseup(function (e) {
    ...
    $('#mswp-container').bind('contextmenu', function () {
        return false;
    }); //阻止默认右击事件
});
...
function init(x, y, mine) {
    ...
    $('#mswp-container').html(block).width(x * 20 + 1).height(y * 20 + 1).show(); //绘图
    $('#warning').html('');
    $('#mswp-submenu').show();
    $('#lastnum').text(lastNum);
}
...
function timer() {
    if (inGame == 1) { //只在游戏进行中计时
        ...
        if (inGame == 1) setTimeout(function () {
            timer();
        }, 500);
    } else if (inGame == 2) {
        $('#time').text('0');
...
//设置难度
function setDifficulty(sign) {
    var difficulty = $("#difficulty")
    switch (sign) {
        case 1: difficulty.val("1"); break;
        case 2: difficulty.val("2"); break;
        case 3: difficulty.val("3"); break;
        default: difficulty.val("4"); break;
    }
}